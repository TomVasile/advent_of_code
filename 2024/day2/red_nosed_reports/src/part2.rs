...
// Moved main from part 1 to this function
// tweaked my good bad checks
// call the bruteforce function and pray!
fn safety_check(matrix: &Vec<Vec<i32>>) -> (usize, usize, Vec<Vec<i32>>) {
    ...
    (bad_row_count, good_row_count, bad_rows)
}
...
fn main() {
    ...
    let (_bad_row_count, good_row_count, bad_rows) = safety_check(&matrix);

    //dbg!(&bad_rows); //good now, i see 4

    let mut part2_good_row_count = 0;

    for bad_row in &bad_rows {
        //println!("{:?}", bad_row);
        for i in 0..bad_row.len() {
            let mut damped = bad_row.clone();
            damped.remove(i);
            //println!("{:?}", damped);
            let (_, good_row, _) = safety_check(&vec!(damped));
            if good_row  > 0 {
                part2_good_row_count += good_row;
                break;
            }
        }
    }
    ...
    println!("Good reports: {}", good_row_count + part2_good_row_count);
}
